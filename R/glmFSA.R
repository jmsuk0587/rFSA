...
glmFSA=function(formula,data,fixvar=NULL,quad=F,m=2,numrs=1,save_solutions=T,fam,cores=1,interactions=T,...){
  ...
  fsa<-function(i,history,...){
    ...
    while(!identical(cur,last)){
      ...
      pval<-tmp[which.min(tmp[,2]),2]
      numswap<-numswap+1
    }
    history[i,(1+m):(2*m)]<-cur
    history[i,(dim(history)[2]-1)]<-pval
  ...
  colnames(solutions)[dim(solutions)[2]:(dim(solutions)[2]-1)]=c("swaps","pval")
  colnames(solutions)[1:m]=paste("start",1:m,sep=".")
  colnames(solutions)[(m+1):(m*2)]=paste("best",1:m,sep=".")
  solutions$pval<-as.numeric(levels(solutions$pval))[solutions$pval]
